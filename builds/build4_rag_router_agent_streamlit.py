...
def normalize_tool_return(tool_name: str, result: Any) -> ToolResult:
    ...
    Consistent handling of different return formats from tools, ensuring that we always have a text
    description and a list of artifact (output) paths, even if the tool returns just a string or a dict
    with varying keys.

    This function allows tool developers to return results in a flexible way while keeping the router's
    expectations consistent. It also ensures that artifact paths are always returned as a list,
    which simplifies downstream handling.

    Accepted return types:
        - str
        - dict with 'text' and 'artifact_paths'
        - tuple(text, artifact_paths)

    This function guarantees:
    1) every tool produces text
    2) every tool produces artifacts consistently
    3) your CLI never breaks on unexpected outputs
    """
...
def build_codegen_chain(
    ...
    system_text = (
        ...
        "Your script MUST:\n"
        "\n"
        "1. PRINT a readable summary to stdout\n"
        "2. SAVE a file named: analysis_summary.txt in report_dir\n"
        "3. SAVE tables as .csv when applicable\n"
        "4. SAVE figures as .png when applicable\n"
        "5. PRINT all saved file paths at the end\n"
        "\n"
        "FORMAT:\n"
        "\n"
        "=== ANALYSIS SUMMARY ===\n"
        ...
        "- Planned artifacts (files to be saved): ...\n"
        "CODE:\n"
        "```python\n"
        "# full executable script# full executable script\n"
    )
    if memory:
        prompt = ChatPromptTemplate.from_messages(
            [
...
def build_results_summarizer_chain(
    ...
    "Important dataset context: This dataset is McDonald's monthly financial data from 2005–2024.\n"
    "Each row represents a monthly time period for McDonald's, NOT a different company.\n"
    "Do not refer to rows as companies, firms, or observations across companies.\n"
    "Interpret results as changes or variation over time for McDonald's.\n\n"
...
def do_router(
    *,
    ...
    if mode == "answer":
        print("\n=== RAG ANSWER ===\n")
        print(note or "No answer returned.")
        print()
        return

    print("\nERROR: Router 'mode' must be 'tool', 'codegen', or 'answer'. Try again.\n")
    print("Raw output was:\n", raw, "\n")
# --------------------------------------------------------------------------------------
# Main
# --------------------------------------------------------------------------------------
...
def initialize_build4_backend(
    ...
    tags = tags or ["streamlit", "build4"]

    data_path = Path(data_path)
    report_dir = Path(report_dir)
    ensure_dirs(report_dir)
    session_dirs = get_session_artifact_dirs(report_dir, session_id)
    ...
    rag_index = None
    if knowledge_dir:
        rag_index = load_saved_rag_index(Path(knowledge_dir))

    suggest_chain = build_suggest_chain(model, temperature, False, True)
    codegen_chain = build_codegen_chain(model, temperature, False, True)
    ...
    summarize_chain = build_results_summarizer_chain(model, temperature, False)

    base_config = make_langfuse_config(session_id=session_id, tags=tags)
    script_path = session_dirs["generated_session"] / "streamlit_generated_analysis.py"
...
def ui_run_router(agent, request):

    router_request = (
        "IMPORTANT CONTEXT: This dataset is McDonald's monthly financial data from 2005–2024. "
        "Each row is a month/time period for McDonald's, NOT a separate company or firm. "
        "Interpret all results as McDonald's financial metrics over time. "
        "Do not say companies, firms, or across companies. "
...
def ui_plan_tool(agent, request):
    raw = traced_toolplan(
        agent["toolplan_chain"],
        agent["schema_text"],
        request,
        agent["base_config"],
        agent["tags"],
...
def ui_run_tool_from_plan(agent, request, plan):
    tool_name = plan.get("tool")
    tool_args = coerce_tool_args(plan.get("args", {}))

    if tool_name == "summarize_numeric":
        if not tool_args.get("numeric_cols") and not tool_args.get("column"):
            tool_args["numeric_cols"] = list(
                agent["df"].select_dtypes(include="number").columns
            )

    if tool_name not in agent["tools"]:
        return {"error": f"Tool {tool_name} not found."}

    unknown_cols = find_unknown_columns(tool_args, agent["df_columns"])
    if unknown_cols:
        return {"error": f"Unknown columns: {sorted(unknown_cols)}"}
    ...
    try:
        ...
        clean_text = (
            res.text
            .replace("companies", "monthly observations")
            .replace("Companies", "Monthly observations")
            .replace("firms", "monthly observations")
            .replace("Firms", "Monthly observations")
        )
...
def ui_run_codegen(agent, request):
    codegen_request, rag_context = prepare_codegen_request_with_rag(
        req=request,
        schema_text=agent["schema_text"],
    ...
    plan, _, verify = split_sections(out)
...
def ui_save_generated_code(agent, code):
    if not code:
        return {"error": "No code found to save."}

    save_text(agent["script_path"], code)
    agent["state"]["code_approved"] = code

    return {"saved_path": str(agent["script_path"])}


def ui_run_saved_code(agent):
    result = run_generated_script(
        agent["script_path"],
        agent["data_path"],
        agent["report_dir"],
